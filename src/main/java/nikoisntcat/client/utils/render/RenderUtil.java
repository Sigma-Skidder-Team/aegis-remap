...
public class RenderUtil extends MinecraftUtil {
    ...
    public static void method1574(DrawContext context, float x1, float y1, float x2, float y2, float z, int color) {
        RenderLayer gui = RenderLayer.getGui();
        Matrix4f pos = context.getMatrices().peek().getPositionMatrix();
        if (x1 < x2) {
            float var9 = x1;
            x1 = x2;
            x2 = var9;
        ...
        VertexConsumer buff = context.vertexConsumers.getBuffer(gui);
        buff.vertex(pos, x1, y1, z).color(color);
        buff.vertex(pos, x1, y2, z).color(color);
        buff.vertex(pos, x2, y2, z).color(color);
        buff.vertex(pos, x2, y1, z).color(color);
    }
    ...
    public static void method1578(MatrixStack matrixStack, float x, float y, float width, float height, float radius, int color, float borderWidth, int borderColor) {
        ...
        shader.setUniformFloats("Radius", radius);
        ...
        buff.vertex(pos, x + width, y + height, 0.0F).texture(1.0F, 1.0F).color(var9, var10, var11, var12);
        buff.vertex(pos, x + width, y, 0.0F).texture(1.0F, 0.0F).color(var9, var10, var11, var12);
        buff.vertex(pos, x, y, 0.0F).texture(0.0F, 0.0F).color(var9, var10, var11, var12);
        BufferRenderer.drawWithGlobalProgram(Objects.requireNonNull(buff.endNullable()));
        RenderSystem.disableBlend();
    }
